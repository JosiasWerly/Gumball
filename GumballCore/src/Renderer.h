...
public:
    ...
    void setup(unsigned int shader, vector<float> data, vector<unsigned int> index) {
        this->shader = shader;
        glDCall(glBindVertexArray(vba)); // resolve what the data is
        glDCall(glBindBuffer(GL_ARRAY_BUFFER, vbo));//ONLY DATA
    ...
    void draw() {
        ...
        //glDCall(glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vea)); this is not needed ?
